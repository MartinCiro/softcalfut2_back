...
export default class AfiliadosAdapter implements AfiliadosPort {
  ...
  async crearAfiliados(afiliadoData: AfiliadoData) {
    try {
      ...
      const estadoActu = estado || 'Activo';
      // Verificar que el equipo existe
      const equipoExistente = await prisma.equipo.findUnique({ where: { id: equipo as number } });
      ...
      const lugarExistente = await prisma.lugarEncuentro.findUnique({ where: { id: lugar_entrenamiento as number } });
      ...
      if (typeof estadoActu === 'string' && estadoActu.toLowerCase() === 'activo') {
        estadoAfiliado = await prisma.estado.upsert({
          where: { nombre: estadoActu },
          update: {},
          create: { nombre: estadoActu },
          select: { id: true }
        })
        estadoAfiliado = estadoAfiliado.id;
      } else {
        estadoAfiliado = estadoActu;
      }

      // Crear el registro en AfiliadoInfo
      await prisma.afiliadoInfo.create({
        data: {
          equipo_id: equipo as number,
          lugar_entrenamiento_id: lugar_entrenamiento as number,
          estado_id: estadoAfiliado,
          logo: logoUri
        },
        include: {
          equipo: {
  ...
  async obtenerAfiliados(): Promise<any> {
    try {
      ...
      const afiliadosParseados = afiliados.map((afiliado) => ({
        id: afiliado.equipo.id, // Cambiado de afiliado.afiliado.id
        logo: afiliado.logo,
        president: afiliado.equipo.usuario
          ? `${afiliado.equipo.usuario.nombres} ${afiliado.equipo.usuario.apellido}`
          : 'No asignado',
        phone: afiliado.equipo.usuario?.num_contacto || 'No registrado',
        categories: afiliado.equipo.categoria
          ? [afiliado.equipo.categoria.nombre_categoria]
          : ['Sin categoría'],
    ...
    try {
      ...
      if (lugar_entrenamiento && lugar_entrenamiento !== afiliadoExistente.lugar_entrenamiento_id) {
        const lugarExistente = await prisma.lugarEncuentro.findUnique({
          where: { id: lugar_entrenamiento as number },
          select: { id: true }
        });
